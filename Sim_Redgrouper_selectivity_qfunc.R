#-------------------------------------------
# sim of q vs selectivity with catchability function
#-------------------------------------------
...
# Method 1: Calibrate catchability from target fishing mortality
calibrate_catchability <- function(target_F, effort, max_selectivity = 1.0) {
  # F = q × E × S
  # Solving for q: q = F / (E × S)
  q <- target_F / (effort * max_selectivity)
  return(q)
}

# Method 2: Estimate catchability from catch and abundance data
estimate_q_from_catch <- function(catch, effort, selectivity, abundance, M = 0.2) {
  # Remove zero or NA values
  valid <- !is.na(catch) & !is.na(abundance) & abundance > 0 & effort > 0 & selectivity > 0
  
  if (sum(valid) == 0) return(NA)
  
  catch <- catch[valid]
  effort <- effort[valid]
  selectivity <- selectivity[valid]
  abundance <- abundance[valid]
  
  # Estimate F from Baranov catch equation: C = F/Z × N × (1 - exp(-Z))
  # For small F, approximate: C ≈ F × N
  # More accurate: solve Z = F + M, then F = Z × C / (N × (1 - exp(-Z)))
  
  q_estimates <- numeric(length(catch))
  for (i in seq_along(catch)) {
    # Iteratively solve for F
    F_est <- optimize(function(F) {
      Z <- F + M
      predicted_catch <- F/Z * abundance[i] * (1 - exp(-Z))
      (predicted_catch - catch[i])^2
    }, interval = c(0, 5))$minimum
    
    # Calculate q
    q_estimates[i] <- F_est / (effort[i] * selectivity[i])
  }
  
  # Return mean q (removing outliers)
  q_mean <- mean(q_estimates[q_estimates > 0 & q_estimates < 10], na.rm = TRUE)
  return(q_mean)
}

# Method 3: Estimate from CPUE and abundance (if CPUE available)
estimate_q_from_cpue <- function(cpue, selectivity, abundance) {
  # CPUE = q × S(L) × N
  valid <- !is.na(cpue) & !is.na(abundance) & abundance > 0 & selectivity > 0
  
  if (sum(valid) == 0) return(NA)
  
  q <- cpue[valid] / (selectivity[valid] * abundance[valid])
  return(mean(q, na.rm = TRUE))
}

# Method 4: Leslie depletion method
estimate_q_leslie <- function(cpue_series, cumulative_catch) {
  # Remove NA values
  valid <- !is.na(cpue_series) & !is.na(cumulative_catch)
  
  if (sum(valid) < 3) return(NA)  # Need at least 3 points
  
  # CPUE_t = q × N_0 - q × CumulativeCatch_t
  model <- lm(cpue_series[valid] ~ cumulative_catch[valid])
  
  if (is.na(coef(model)[2])) return(NA)
  
  q <- -coef(model)[2]  # Negative of slope
  return(max(q, 0))  # Ensure positive
}

# Method 5: From exploitation rate (if known from tagging or other studies)
estimate_q_from_exploitation <- function(exploitation_rate, effort, selectivity, M = 0.2) {
  # u = F/Z × (1 - exp(-Z))
  # Solve for F, then q = F / (E × S)
  
  if (exploitation_rate <= 0 || exploitation_rate >= 1) return(NA)
  
  # Iteratively solve for F
  F_est <- optimize(function(F) {
    Z <- F + M
    predicted_u <- F/Z * (1 - exp(-Z))
    (predicted_u - exploitation_rate)^2
  }, interval = c(0, 5))$minimum
  
  q <- F_est / (effort * selectivity)
  return(q)
}
...
# Example 1: Calibrate catchability from target F
# If we want F = 0.3 at full selectivity with effort = 1.0
q_calibrated <- calibrate_catchability(target_F = 0.3, effort = 1.0)
cat("\n=== CATCHABILITY ESTIMATION ===\n")
cat("Calibrated catchability (for F=0.3, E=1.0):", round(q_calibrated, 4), "\n\n")
...
sel_data <- data.frame(
  ...
  selectivity = c(
    logistic_selectivity(length_seq, SL50 = 25, SL95 = 55, By = 0.1),
    dome_selectivity(length_seq, 35, 0.3, 80, 0.3)
  ),
...
# ============================================================================
# EXAMPLE: ESTIMATE CATCHABILITY FROM SIMULATED DATA
# ============================================================================

cat("\n=== CATCHABILITY ESTIMATION FROM SIMULATED DATA ===\n\n")

# Use the logistic fishing scenario to back-calculate catchability
final_year_data <- logistic_fishing %>% filter(year == 50)

# Get catch by length class
catch_by_length <- final_year_data %>%
  mutate(catch_total = catch_female + catch_male,
         abundance_total = N_female + N_male)

# Estimate q from catch data (using multiple length classes)
q_estimated <- estimate_q_from_catch(
  catch = catch_by_length$catch_total,
  effort = 0.5,  # The effort used in simulation
  selectivity = catch_by_length$selectivity,
  abundance = catch_by_length$abundance_total,
  M = M
)

cat("Estimated catchability from catch data:", round(q_estimated, 4), "\n")
cat("Original catchability used in simulation: 0.4\n")
cat("Difference:", round(abs(q_estimated - 0.4), 4), "\n\n")

# Example: Estimate q from exploitation rate
# Calculate overall exploitation rate
total_catch <- sum(catch_by_length$catch_total)
total_abundance <- sum(catch_by_length$abundance_total)
exploitation_rate <- total_catch / total_abundance

# Estimate q from this exploitation rate
mean_selectivity <- weighted.mean(catch_by_length$selectivity, 
                                  catch_by_length$abundance_total)
q_from_exploitation <- estimate_q_from_exploitation(
...
cat("Exploitation rate observed:", round(exploitation_rate, 4), "\n")
cat("Estimated catchability from exploitation rate:", 
    round(q_from_exploitation, 4), "\n\n")

cat("Note: These estimation methods can help you calibrate catchability\n")
cat("when you have real fishery data (catch, effort, and abundance estimates).\n")
